...
import DetalleClienteImport, { Registro } from './DetalleClienteImport'
...
export const ImportClientsModal = ({ open, onClose }: Props): ReactElement => {
  ...
  const upload = async () => {
    ...
    setStatus('cargando')
    try {
      setMessage(t('Importando clientes...'))
      const { fileUploadPromise, key } = await putFile('clientes', '_get_import_url', file)
      setProgress(5)
      ...
      debug('subscribiendo a', 'clientes-import', key)
      onMessage('clientes-import', key, ({ error, register, status, progress, message }) => {
        if (error && !register) {
          setProgress(() => 0)
          setFile(() => null)
          setStatus(() => 'error')
          errorMessage(message, 'Error importando clientes.')
  ...
  return (
    <>
      <Dialog open={open}>
        <DialogTitle>{t('Importar Clientes')}</DialogTitle>
        <DialogContent className={classes.content}>
          <Grid item className={classes.instructions}>
            <Typography variant="body1">
              {t('Selecciona un archivo en formato excel con el listado de tus clientes.')}
            </Typography>
