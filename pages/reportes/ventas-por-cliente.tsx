  ...
  const generarReport = async () => {
    ...
    try {
      const { url } = await request<{ url: string }>(
        'POST',
        'facturas',
        `reportes/_export-ventas-por-cliente?rango=${rangoParam}`,
      )
      setUrlDescarga(url)
    } catch (e) {
  ...
  const showReporte: FormEventHandler = async (e) => {
    ...
    try {
      const resp = await request<Venta[]>(
        'GET',
        'facturas',
        `reportes/ventas-por-cliente?rango=${rangoParam}&size=300`,
      )
      setVentas(resp)
    } catch (e) {
      handleError('Error al generar el reporte', 'consulte a su administrador', e)
    ...
    <Page title="Ventas por cliente">
      ...
      <DataGrid
        ...
        columns={[
          ...
          {
            field: 'subtotal',
            headerName: 'Subtotal ventas',
            flex: 1,
            cellClassName: classes.dataGridNumber,
            headerClassName: classes.dataGridColumns,
            valueFormatter: ({ row }) => `$${pesosMxLocale.format(row.subtotal)}`,
            align: 'right',
            headerAlign: 'right',
          },
          {
            ...
            cellClassName: classes.dataGridNumber,
            headerClassName: classes.dataGridColumns,
            valueFormatter: ({ row }) => `$${pesosMxLocale.format(row.total)}`,
            align: 'right',
            headerAlign: 'right',
          },
        ]}
